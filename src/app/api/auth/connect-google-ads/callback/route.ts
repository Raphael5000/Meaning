 ...
 * Mirrors the helper in connect-google-admin/callback.
 */
function httpsPost(
  url: string,
  body: string,
...
/**
 * GET /api/auth/connect-google-ads/callback
 *
 * Handles the OAuth callback for the adwords scope.
...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL("/?error=ads_oauth_denied", baseUrl)
    );
  }

  const cookieStore = await cookies();
  const storedState = cookieStore.get("ga_ads_state")?.value;
  cookieStore.delete("ga_ads_state");
  ...
  try {
    ...
    if (!tokenRes.ok) {
      console.error("[connect-google-ads] Token exchange failed:", tokenRes.body);
      return NextResponse.redirect(
    ...
    // Update the existing Account with the broader-scoped tokens (now includes adwords)
    ...
    // Reactivate any DataSources that were DISCONNECTED by a provider reset.
    // When the user reconnects OAuth, their existing BigQuery data is still
    // there — no need to force them through enable-export again.
    ...
    if (reactivated.count > 0) {
      console.log(`[connect-google-ads] Reactivated ${reactivated.count} DataSource(s) after reconnect`);
    }

    return NextResponse.redirect(
      new URL("/?ads_connected=true", baseUrl)
    );
  } catch (err) {
